...
class PubSubRequest(generic.Stanza):
    ...
    _requestVerbMap = {
        ...
        # nfvs:
        ('get', NS_PUBSUB, 'configure'): 'configureGet',
        ('set', NS_PUBSUB, 'configure'): 'configureSet',
        #
    ...
    def _parse_configureOrNone(self, verbElement):
        ...
        for element in verbElement.parent.elements():
            if element.uri == NS_PUBSUB and element.name == 'configure':
                ...
                if ('pubsub#node_type' in form.fields and
                    form.fields['pubsub#node_type'].values[0] == 'collection'):
...
class PubSubService(XMPPHandler, IQHandlerMixin):
    ...
    _legacyHandlers = {
        ...
        # nfvs (added subscribe options)
        'subscribe': ('subscribe', ['sender', 'recipient',
                                   'nodeIdentifier', 'subscriber', 'options']),
        ...
        # nfvs
        'optionsSet': ('setOptions', ['recipient', 'nodeIdentifier',
                                      'subscriber', 'options',
                                      'subscriptionIdentifier', 'sender']),
        'optionsGet': ('getOptions', ['recipient', 'nodeIdentifier',
                                      'subscriber', 'subscriptionIdentifier',
                                      'sender']),
        'affiliationsGet': ('getAffiliations', ['sender', 'recipient',
                                                'nodeIdentifier']),
    ...
    def getDiscoInfo(self, requestor, target, nodeIdentifier=''):
        ...
        d = defer.maybeDeferred(getInfo, requestor, target,
                                nodeIdentifier or '')
        ...
        if resource is not self:
            ...
            if 'options' in argNames and request.options:
                ...
                if request.verb in ['create']:
                    ...
                    args[argNames.index('options')]['pubsub#node_type'] = \
                            nodeType
